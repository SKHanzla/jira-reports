    ...
    <main style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 28 }}>
          <svg width="64" height="40" viewBox="0 0 72 44" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ marginBottom: 8 }}>
            <path d="M36 38 C30 30, 14 26, 4 14 C10 18, 18 20, 24 24 C18 16, 10 10, 6 4 C14 10, 22 16, 28 22 C24 14, 20 8, 18 2 C24 10, 28 18, 32 26 C30 18, 30 10, 30 4 C32 12, 34 20, 36 28" fill="#a855b5"/>
            <path d="M36 38 C42 30, 58 26, 68 14 C62 18, 54 20, 48 24 C54 16, 62 10, 66 4 C58 10, 50 16, 44 22 C48 14, 52 8, 54 2 C48 10, 44 18, 40 26 C42 18, 42 10, 42 4 C40 12, 38 20, 36 28" fill="#7c3aab"/>
            <line x1="36" y1="28" x2="36" y2="42" stroke="#a855b5" strokeWidth="2.5" strokeLinecap="round"/>
          </svg>
          <div style={{ display: "flex", alignItems: "baseline", letterSpacing: "0.18em", fontSize: 15, textTransform: "uppercase", marginBottom: 4 }}>
        ...
        <h2 style={{ fontSize: 20, fontWeight: 700, color: "#1c1917", marginBottom: 4, textAlign: "center" }}>Sign in</h2>
        <p style={{ fontSize: 13, color: "#78716c", textAlign: "center", marginBottom: 24 }}>CDEF Report Portal</p>
...
export default function Page() {
  ...
  const totalEst = result?.rows?.reduce((s, r) => s + r.original_estimate, 0) ?? 0;
  const totalLogged = result?.rows?.reduce((s, r) => s + r.total_hours_logged, 0) ?? 0;
    ...
    <main style={{
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 36, position: "relative" }}>
          <svg width="80" height="50" viewBox="0 0 72 44" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ marginBottom: 8 }}>
            <path d="M36 38 C30 30, 14 26, 4 14 C10 18, 18 20, 24 24 C18 16, 10 10, 6 4 C14 10, 22 16, 28 22 C24 14, 20 8, 18 2 C24 10, 28 18, 32 26 C30 18, 30 10, 30 4 C32 12, 34 20, 36 28" fill="#a855b5"/>
            <path d="M36 38 C42 30, 58 26, 68 14 C62 18, 54 20, 48 24 C54 16, 62 10, 66 4 C58 10, 50 16, 44 22 C48 14, 52 8, 54 2 C48 10, 44 18, 40 26 C42 18, 42 10, 42 4 C40 12, 38 20, 36 28" fill="#7c3aab"/>
            <line x1="36" y1="28" x2="36" y2="42" stroke="#a855b5" strokeWidth="2.5" strokeLinecap="round"/>
          </svg>
          <div style={{ display: "flex", alignItems: "baseline", letterSpacing: "0.18em", fontSize: 15, textTransform: "uppercase" }}>
            <span style={{ fontWeight: 400, color: "#6b7280" }}>EXIT</span>
            <span style={{ fontWeight: 700, color: "#1c1917" }}>BLISS</span>
          </div>
          <div style={{ width: 48, height: 2, background: purpleMid, marginTop: 4 }} />
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#1c1917", marginBottom: 4, letterSpacing: "-0.01em" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 24, alignItems: "center" }}>
          ...
          <button
            ...
            style={{
              ...
              border: status === "done" ? `1.5px solid #c084d4` : "1.5px solid #e5e7eb",
              borderRadius: 8,
              padding: "10px 22px",
              fontSize: 13,
              fontWeight: 600,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{
              ...
              background: status === "done" ? "#639922" : status === "error" ? "#E24B4A" : status === "loading" ? purpleMid : "#d1d5db",
              animation: status === "loading" ? "pulse 1s infinite" : "none",
            }} />
            <span style={{ fontSize: 12, color: "#78716c" }}>
              {status === "idle" ? "Ready" : status === "loading" ? "Fetching from Jira..." : status === "done" ? "Report ready" : "Error"}
            </span>
          </div>
        </div>
        ...
        {status === "error" && (
          <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 8, padding: "14px 18px", marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 13, color: "#ef4444" }}>{result?.error ?? "Unknown error"}</p>
          </div>
        )}
        ...
        {status === "done" && result?.rows && (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 12, marginBottom: 24 }}>
              {[
                { label: "Stories", value: String(result.storyCount ?? 0), color: "#1c1917", accent: false },
                { label: "Estimate", value: `${totalEst.toFixed(1)}h`, color: "#1c1917", accent: true },
                { label: "Logged", value: `${totalLogged.toFixed(1)}h`, color: "#15803d", accent: false },
                { label: "Remaining", value: `${totalLeft.toFixed(1)}h`, color: "#92400e", accent: false },
              ].map((s) => (
                <div key={s.label} style={{
                  background: "#fff",
                  border: `1px solid ${s.accent ? "#c084d4" : "#e9d5f7"}`,
                  borderTop: s.accent ? `3px solid ${purpleMid}` : `3px solid #e9d5f7`,
                  borderRadius: 10,
                  padding: "14px 16px",
                }}>
                  <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", color: "#a8a29e", marginBottom: 6 }}>
                    {s.label}
                  </div>
            ...
            <div style={{ background: "#fff", border: "1px solid #e9d5f7", borderRadius: 10, overflow: "hidden", marginBottom: 16 }}>
              <div style={{
                ...
                <span style={{ fontSize: 12, color: "#9333ea" }}>{result.rows.length} total rows in XLSX</span>
              </div>
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: "#faf5ff" }}>
                      {["Epic Name", "Story Name", "Project", "Est (h)", "Logged (h)", "Left (h)"].map((h) => (
                        <th key={h} style={{
                  ...
                  <tbody>
                    {result.rows.slice(0, 20).map((row, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid #faf5ff" }}>
                        <td style={{ padding: "9px 14px", color: "#78716c", maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {row.epic_name}
                        </td>
                        <td style={{ padding: "9px 14px", color: "#1c1917", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {row.story_name}
                        </td>
                        <td style={{ padding: "9px 14px" }}>
                          <span style={{ background: purpleLight, color: purple, padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600 }}>
                            {row.project_key}
                          </span>
                        </td>
                        <td style={{ padding: "9px 14px", color: "#1c1917", textAlign: "right" }}>{row.original_estimate}</td>
                        <td style={{ padding: "9px 14px", color: "#15803d", textAlign: "right", fontWeight: 600 }}>{row.total_hours_logged}</td>
                        <td style={{ padding: "9px 14px", color: row.hrs_left === 0 ? "#a8a29e" : "#92400e", textAlign: "right", fontWeight: 600 }}>{row.hrs_left}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <p style={{ fontSize: 12, color: "#a8a29e", textAlign: "center" }}>
              XLSX includes all {result.rows.length} rows · Active_CDEF_Monthly_Report_{month}_{year}.xlsx
            </p>
          </>
        )}
      </div>
